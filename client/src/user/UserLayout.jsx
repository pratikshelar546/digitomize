...
export default function UserLayout() {
  ...
  const handleLogout = () => {
    signOut()
    navigate('/login?message=sucessfully logged out')
  }
  return (
    <>
      <ToastContainer />
      <div className="w-full flex justify-end px-4 py-4">
        <button onClick={handleLogout} className="relative inline-flex items-center justify-center px-6 py-2 text-lg font-medium tracking-tighter text-white bg-gray-800 rounded-md group">
          ...
          <span className="relative text-black transition-colors duration-200 ease-in-out delay-100 group-hover:text-white">Logout</span>
        </button>
      </div>
      {/* <UserHeader /> */}
      <Outlet />
